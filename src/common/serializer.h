...
class SerializerStatic
{
	...
	static void append32u(uint32_t x, std::vector<uint8_t> &data)
	{
		if (x < uint32_size_1)
	...
	static void append64u(uint64_t x, std::vector<uint8_t>& data)
	{
		if (x < uint64_size_1)
			data.push_back((uint8_t) x);
	...
	static uint32_t load32u(std::vector<uint8_t>::iterator &iter)
	{
		uint32_t r = 0;
	...
	static uint64_t load64u(std::vector<uint8_t>::iterator &iter)
	{
		uint64_t r = 0;
...
class Serializer
{
	...
	using model_no_reps_t = refresh::rc_simple_fixed<refresh::vector_io_stream, 5, 1 << 12, 2>;	// 0, same_as_before, 1B, 2B, 3B
	refresh::rc_context_vec<model_no_reps_t> dict_no_reps;
...
public: 
	...
	void set_data(std::vector<uint8_t> &&_data);
	
	void append64u(uint64_t x);
	void append64i(int64_t x);
	void append64f(double x);
	void append32u(uint32_t x);
	void append32i(int32_t x);
	void append32f(float x);
	void append16u(uint16_t x);
	...
	void append_vec_str(const std::vector<std::string>& vs);
	...
	void load_vec_str(std::vector<std::string>& vs, size_t n_items);
...
#ifdef USE_ZSTD
	// Compression methods
	void compress_zstd(ZSTD_CCtx *zstd_cctx = nullptr, std::vector<char>* zstd_dict = nullptr);
	void compress_zstd_rc(ZSTD_CCtx* zstd_cctx, std::vector<char>* zstd_dict, rc_encoder<vector_io_stream>* rce);
	void decompress_zstd(ZSTD_DCtx* zstd_dctx = nullptr, std::vector<char>* zstd_dict = nullptr);
	void decompress_zstd_rc(ZSTD_DCtx* zstd_dctx, std::vector<char>* zstd_dict, rc_decoder<vector_io_stream>* rcd);
#endif
};
