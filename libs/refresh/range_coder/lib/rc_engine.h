...
public:
	...
	void encode_1bit(uint32_t bit)
	{
		encode_frequency_pow2(1, bit, 1);
	}

	void encode_frequency_pow2(freq_t symFreq_, freq_t cumFreq_, freq_t log2_totalFreqSum_)
	{
		assert(range > (1ull >> log2_totalFreqSum_));
		range >>= log2_totalFreqSum_;
		low += range * cumFreq_;
		range *= symFreq_;
...
//			uint64_t diff = (low | TopValue) - low;
//			range -= diff * (((low ^ (low + range)) & Mask) != 0);
...
public:
	...
	freq_t get_cumulative_freq_1bit()
	{
		return (freq_t)(buffer / (range >>= 1));
	}

	freq_t get_cumulative_freq_pow2(freq_t log2_totalFreq_)
	{
//		assert(totalFreq_ != 0);
		return (freq_t)(buffer / (range >>= log2_totalFreq_));
	}
