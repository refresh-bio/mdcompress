...
public:
	vector_io_stream(std::vector<uint8_t>& _v) : v(_v), read_pos(0)
	{}
...
public:
	...
	void GetOutput(std::vector<uint8_t>& _v_io)
	{
		_v_io = move(v_io);
	}

	// Setting compressed input (before Init)
	void SetInput(std::vector<uint8_t>& _v_io)
	{
		v_io = move(_v_io);
	}
};
