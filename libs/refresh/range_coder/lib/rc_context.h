...
{
	std::array<rc_context_vec<MODEL>, SIZE> models;
...
public:
	...
	std::pair<uint32_t, MODEL*> find(const uint32_t model_id, const rc_context_t ctx)
	{
		if (model_id >= SIZE)
...
template<typename MODEL, unsigned int SIZE> class rc_context_arr_vec_emb {
	std::array<rc_context_vec_emb<MODEL>, SIZE> models;
